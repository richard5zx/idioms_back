...
public interface IdiomExampleRepository extends JpaRepository<IdiomExample, Integer> {
	...
	@Query(value="SELECT * FROM idiomExample WHERE example LIKE %?1%", nativeQuery=true)
	public List<IdiomExample> findByExample(String example);
	
	@Query(value="SELECT * FROM idiomExample WHERE date_time BETWEEN ?1 AND ?2", nativeQuery=true)
